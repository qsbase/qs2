...
if(algo == "qs-legacy") {
  ...
  now()
  x <- qs2::qread(file_path, nthreads = nthreads)
  read_time <- later()
} else if(algo == "qdata") {
  suppressPackageStartupMessages( library(qs2, quietly=TRUE) )
  now()
  x <- qs2::qdread(file_path, nthreads = nthreads)
  read_time <- later()
} else if(algo == "fst") {
  ...
  options(arrow.use_altrep = FALSE) # don't use ALTREP/mmap cheats for benchmark
