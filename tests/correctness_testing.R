...
qs_save_rand <- function(x, file, format, check) {
  if(qs2:::check_TBB() == FALSE) { # no TBB support
    nt <- 1
  } else if (nzchar(R_TESTS)) { # running from CMD CHECK, use max 2 threads
    nt <- sample(2,1)
  } else {
    nt <- sample(5,1)
  }
  ...
  cl <- sample(10,1)
  if(format == "qs2") {
    qs2::qs_save(x, file = file, compress_level = cl, store_checksum = check, nthreads = nt)
  } else if(format == "qdata") {
    qs2::qd_save(x, file = file, compress_level = cl, store_checksum = check, nthreads = nt)
  }
}

qs_read_rand <- function(file, format, check) {
  ...
  if(format == "qs2") {
    qs2::qs_read(file, validate_checksum=check, nthreads = nt)
  } else if(format == "qdata") {
    qs2::qd_read(file, use_alt_rep = ar, validate_checksum=check, nthreads = nt)
  }
}
...
for (q in 1:reps) {
  ...
  for (tp in test_points) {
    for (i in 1:3) {
      ...
      x1 <- c(NA, "", x1)
      time[i] <- Sys.time()
      qs_save_rand(x1, myfile, format, check)
      z <- qs_read_rand(myfile, format, check)
  ...
  test_points_ext <- c(test_points, seq(2^19-1000,2^19)) %>% sort
  time <- vector("numeric", length = 3)
  for (tp in test_points_ext) {
    for (i in 1:3) {
      x1 <- list(raw(tp), a="a")
      time[i] <- Sys.time()
      qs_save_rand(x1, myfile, format, check)
